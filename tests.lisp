...
((progn (let* ((foo 3)) `(bar ~@(list foo 1)))) '( bar 3 1) )
((progn (destructuring-bind-parse 'target 0))
 '((target 0)) )
((progn (destructuring-bind-parse '(targeta targetb) 'value))
 '((targeta (nth 0 value)) (targetb (nth 1 value))) )
((progn (destructuring-bind-parse '(targeta (targetb targetc)) 'value))
 '((targeta (nth 0 value)) (targetb (nth 0 (nth 1 value))) (targetc (nth 1 (nth 1 value)))))
((progn (destructuring-bind-parse '((targeta targetb) targetc) 'value))
 '((targeta (nth 0 (nth 0 value))) (targetb (nth 1 (nth 0 value))) (targetc (nth 1 value))))
...
((progn (def targeta)
 ...
 '(1 2) )
((progn (let* ((l (list 1))) (setf (aref l 0) 0) l))
 '(0) )
((progn (let* ((l (dict :foo 1))) (setf (aref l :foo) 0) l))
 (dict :foo 0 ) )
((progn (let ((foo 3)) foo)) 3 )
((progn (let (((:= foo bar) (list 0 1))) (list foo bar)))
 '(0 1) )
((progn (let ((foo 3)) `(bar (+ ~foo 1))))
 '( bar (+ 3 1)) )
((progn (nth 1 '(1 2 3))) 2 )
((progn (car '(1 2 3))) 1 )
((progn (cdr '(1 2 3))) '(2 3) )
((progn (reversed '(1 2 3))) '( 3 2 1) )
((progn (let ((l '())) (cons:append l (list 3)))) '(3) )
((progn (let ((l '(1))) (cons:append l (list 3)))) '(1 3) )
((progn (+ 1 2)) 3 )
((progn `(list ~1))
 '(list 1))
((progn
   (defmacro test (a) `(list ~a))
   (test 1))
 '(1))
...
((progn
 ...
 3 )
((progn ((fn (a) a) (+ 1 2))) 3 )
((progn ((fn (a) a) (car '(3 2 1)))) 3 )
((progn ((fn (a) a) (cdr '(3 2 1)))) '( 2 1) )
...
((progn (defun foo (&keys ks) ks) (foo :a 1))
 (dict :a 1 ) )
((progn (defun foo (&keys ks) ks) (foo :a 1 :b 2 :c 3))
 (dict :a 1  :b 2  :c 3 ) )
((progn (defun foo (a &keys ks) (list a ks)) (foo 1 :b 2))
 (list  1 (dict :b 2)) )
((progn (defun foo (a &keys ks) (list a ks)) (foo :a 1 :b 2 :c 3))
 (list 1 (dict :b 2 :c 3)))
((progn (defmacro add (a) (list '+ 5 a)) (add 1))
 6 )
;; FIXME emulate struct structure
;; ((progn (defstruct Foo) (Foo)) (Foo) )
;; ((progn (defstruct Foo a) (Foo 1)) (Foo :a 1) )
;; ((progn (defstruct Foo a) (def v (Foo 1)) (Foo-a v)) 1 )
((progn (def foo (dict :a 1)))
 (dict :a 1 ) )
((progn (let ((n 0)) (dolist (i '(0 1 2 3 4)) (set n (+ n i))) n))
 10 )
((progn (let ((n 3))
          (dolist (i '())
            (set n (+ n i)))
...
((progn (let ((r '()))
          (dolist (i '(0 1 2 3 4))
            (set r (cons:append r (cons i nil))))
          r))
 '( 0 1 2 3 4) )
...
((progn
   (defmacro foo (a b c)
     `(bar 1 2 ~c))
   (macroexpand-1 '(foo 4 8 3)))
 '(bar 1 2 3))
;; FIXME specials are not macro-expandable
;; ((progn
...
((progn
   (backquote-internal '(defmacro ~name (var &rest args)
                          `foo)
                       false))
 '(list (list 'defmacro name (list 'var '&rest 'args)
              (list 'backquote 'foo))))
((progn
   (backquote-internal '(defmacro ~name (var &rest args)
...
((progn (macroexpand-1 '(setf (:= targeta targetb) '(0 1))))
 ;; FIXME this should really be a let
 '(progn (def __setf-value-1593 '(0 1))
   (set targeta (nth 0 __setf-value-1593))
   (set targetb (nth 1 __setf-value-1593)))
)
